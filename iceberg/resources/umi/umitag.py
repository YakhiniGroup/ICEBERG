...
def get_umi(r1, r2, i1, i2, umi_bps_amount_from_reads_start):
    molecular_barcode = i2[1][8:16]
    return '%s_%s_%s' % (molecular_barcode,
                         r1[1][0:umi_bps_amount_from_reads_start],
                         r2[1][0:umi_bps_amount_from_reads_start])


def umitag(read1, read2, index1, index2, read1_out, read2_out, out_dir, umi_bps_amount_from_reads_start):
    ...
    for r1,r2,i1,i2 in zip(fq(read1), fq(read2), fq(index1), fq(index2)):
        ...
        molecular_id = get_umi(r1, r2, i1, i2, umi_bps_amount_from_reads_start)
    ...
    cmd = 'cat ' + f'"{r1_umitagged_unsorted_file}"' + ' | paste - - - - | sort -k3,3 -k1,1 | tr "\t" "\n" >' + f'"{read1_out}"'
    subprocess.check_call(cmd, shell=True, env=os.environ.copy())
    cmd = 'cat ' + f'"{r2_umitagged_unsorted_file}"' + ' | paste - - - - | sort -k3,3 -k1,1 | tr "\t" "\n" >' + f'"{read2_out}"'
    subprocess.check_call(cmd, shell=True, env=os.environ.copy())
...
def main():
    ...
    parser.add_argument('--umi_bps_amount_from_reads_start', default=6, choices=range(6, 30))

    args = vars(parser.parse_args())
    out_dir = args['out_dir']

    umitag(read1_in, read2_in, index1, index2, read1_out, read2_out, out_dir, umi_bps_amount_from_reads_start)
