...
def consolidate_position(bases, quals, min_qual, min_freq):
    # if len(bases) == 1:
    #     # print(True, chr(bases[0]), quals[0])
    #     return True, chr(bases[0]), quals[0]
    ...
    for bb, qq in zip(bases, quals):
        ...
        if qq > qual[chr(bb)]:
            qual[chr(bb)] = qq
    most_common_base = max([key for key in num], key=(lambda key: num[key]))
    freq = float(num[most_common_base]) / len(bases)
    if freq > min_freq:
        # print(most_common_base,num)
        return True, most_common_base, qual[most_common_base]
    else:
        #print(most_common_base, num, dict([(key, val) for key, val in qual.items()]), len(bases))
        # print()
        return False, 'N', 0
...
def consolidate(fastq_file, consolidated_fastq_file, min_qual, min_freq):
    ...
    for cur_molecular_id, cur_sample_id, reads in bins:
        ...
        # print([list(read.qual) for read in reads])
        ...
        #print(cur_molecular_id, cur_sample_id)
